    ...
    <div class="flex flex-wrap -m-4">
      @foreach($properties as $property)
       <div class="max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
            <img class="object-cover object-center w-full h-56" src="{{ $propertyDetails->image ?? 'images/img-not-available.jpeg' }}" alt="avatar">
            ...
            <div class="px-6 py-4">
                <h1 class="text-xl font-semibold text-gray-800 dark:text-white">${{ $propertyDetails->price ?? 'No info available' }}</h1>

                <p class="py-2 text-gray-700 dark:text-gray-400">{{ $property->description }}</p>
                ...
                <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                    ...
                    <h1 class="px-2 text-sm">{{ $propertyDetails['address']['line'] ?? 'Address not available'}}, {{ $propertyDetails['address']['city'] ?? '' }}, {{ $propertyDetails['address']['state_code'] ?? '' }}, {{ $propertyDetails['address']['postal_code'] ?? ''}}</h1>
                </div>
                ...
                <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                    ...
                    <h1 class="px-2 text-sm">Baths: {{ $propertyDetails['baths'] ?? 'No info available' }}</h1>
                </div>
                ...
                <div class="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                    ...
                    <h1 class="px-2 text-sm">Beds: {{ $propertyDetails['beds'] ?? 'No info available' }}</h1>
                </div>
